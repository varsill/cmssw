  ...
  for (auto it = geom.beginSensor(); it != geom.endSensor(); ++it) {
    ...
    if(plane==0){//to make sure that these trackHit historgrams will be booked just once
      ...
      if (station == 0) {
        ...
        h1Xi_[detId] =
            ibooker.book1DD(Form("h1Xi_arm%i_st%i_rp%i", arm, station, rp),
                     Form("h1Xi_arm%i_st%i_rp%i;#xi", arm, station, rp), xiBins,
                     xiMin, xiMax);
        h1Tx_[detId] =
            ibooker.book1DD(Form("h1Tx_arm%i_st%i_rp%i", arm, station, rp),
                     Form("h1Tx_arm%i_st%i_rp%i;Tx", arm, station, rp),
                     angleBins, angleMin, angleMax);
        h1Ty_[detId] =
            ibooker.book1DD(Form("h1Ty_arm%i_st%i_rp%i", arm, station, rp),
...
void ReferenceAnalysisDQMWorker::analyze(const edm::Event &iEvent,
  ...
  for (auto &proton : *protons) {
    ...
    for (auto &track_ptr : proton.contributingLocalTracks()) {
      ...
      uint32_t xBin =
          h2RefinedTrackEfficiency_[pixelDetId]->getTH2D()->GetXaxis()->FindBin(trackX0);
      uint32_t yBin =
          h2RefinedTrackEfficiency_[pixelDetId]->getTH2D()->GetYaxis()->FindBin(trackY0);
      double trackEfficiency =
          h2RefinedTrackEfficiency_[pixelDetId]->getTH2D()->GetBinContent(xBin, yBin);
