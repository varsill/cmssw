...
private:
  ...
  void EfficiencyTool_2018DQM::setGlobalBinSizes(int station, CTPPSPixelDetId& rpId);
      ...
      for(auto& arm: armIds)
      {
        for(auto& station: stationIds)
        {
          for(auto& rp: romanPotIds)
          {
            ...
            {
              CTPPSPixelDetId rpId(arm, station, rp, plane);
...
void EfficiencyTool_2018DQM::initialize() {
  ...
  h1BunchCrossing_ = ibooker.book1D("h1BunchCrossing", "h1BunchCrossing",
                              totalNumberOfBunches_, 0., totalNumberOfBunches_);
  h1CrossingAngle_ =
      ibooker.book1D("h1CrossingAngle", "h1CrossingAngle", 70, 100., 170);
...
void EfficiencyTool_2018DQM::getPlaneCombinations(
    const std::vector<uint32_t> &inputPlaneList, uint32_t numberToExtract,
    std::vector<std::pair<std::vector<uint32_t>, std::vector<uint32_t>>>
        &planesExtractedAndNot) {
...
float EfficiencyTool_2018DQM::probabilityNplanesBlind(
...
float EfficiencyTool_2018DQM::efficiencyPartialDerivativewrtPlane(
    uint32_t plane, const std::vector<uint32_t> &inputPlaneList,
    int numberToExtract, const std::map<unsigned, float> &planeEfficiency) {
...
float EfficiencyTool_2018DQM::errorCalculation(
...
float EfficiencyTool_2018DQM::probabilityCalculation(
    const std::map<uint32_t, float> &planeEfficiency) {
...
bool EfficiencyTool_2018DQM::Cut(CTPPSPixelLocalTrack track, int arm,
                              int station) {
