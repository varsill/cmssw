...
then
	...
	{
		cmsRun ${2}/src/RecoCTPPS/RPixEfficiencyTools/test/ReReco.py runNumber=${1} skipEvents=${3} maxEvents=1 \
		...
		if [ $exit_code -ne 0 ]
		then
			lastline=`eval "tail -1 ${2}/src/RecoCTPPS/RPixEfficiencyTools/test/LogFiles/Run${1}_test.out | grep Closed"`
			if [ -z "$lastline" ]
			then 
				fail_line=`eval "tac ${2}/src/RecoCTPPS/RPixEfficiencyTools/test/LogFiles/Run${1}_test.out | grep -m 1 -o /store/.*.root"`
				echo "Excluding file: ${fail_line}"
				fail_line=`echo "$fail_line" | sed -e 's,\\/,\\\\/,g'`
				sed -i -e "s/$fail_line/#$fail_line/" ${2}/src/RecoCTPPS/RPixEfficiencyTools/test/InputFiles/Run${1}.dat
	...
	#${2}/src/RecoCTPPS/RPixEfficiencyTools/test/LogFiles/Run${1}_test.out
fi